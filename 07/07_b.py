...
# Identify smallest directory that can be deleted to increase free space to 30000000
import re
import sys
...
target = spaceneeded - (filesystem - results[-1][1])

holder = results[-1][1]
targetdir = "/"
for r in results:
    if (r[1] > target) and (r[1] < holder):
        targetdir = r[0]
        holder = r[1]

print(targetdir, str(holder))